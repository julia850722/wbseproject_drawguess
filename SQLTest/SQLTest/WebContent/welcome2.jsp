	...
	var  logined = false;
	  ...
	  function testAPI() {
	    ...
	    FB.api('/me', function(response) {
	      ...
	      personID();
		    ...
		    document.getElementById('login').type = 'hidden';
		    document.getElementById('getID').value = response.id;
	    });
	  ...
	  function checking(){
		  ...
		  if(logined == false){
			  ...
			  window.open('canvas.jsp');
			  //document.location.href="canvas.jsp";
			  
		  }
	  }
	  
	  function changeboutton(logined){
		  if(logined == true){
			  document.getElementById('button').typeName = 'submit';
			  //type
		  }
	  }
					...
					<h2 class="title">遊戲介紹</h2>
						<div style="clear: both;">&nbsp;</div>
							<div class="entry">
								...
								<form method="post" action="PlayerSelectServlet" name="start">
								<input name="person_ID" type="hidden" id="getID"></input>
								<button onClick="checking()" type="hidden" id="button" class="box1"><span>開始遊戲</span></button>
								</form>
								<input id="login" onClick="fb_login()" class="box2" value="登入">
								</input>
